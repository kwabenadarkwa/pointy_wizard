...
describe('convert to Pointy lang', () => {
  test('makes sure that the nodes that are put in come out as what they are supposed to be more complex version of the same test', () => {
    const initialNodes: Node[] = [
      { id: '1', data: { label: 'Node 1' }, position: { x: 5, y: 5 } },
      { id: '2', data: { label: 'Node 2' }, position: { x: 5, y: 100 } },
    ...
    const map: string[] = getPointyLang(initialNodes, initialEdges);
    console.log(map);
    const equal = ['Node 1', '->', 'Node 2', '->', 'Node 3'];
    expect(map).toEqual(equal);
  });
});
